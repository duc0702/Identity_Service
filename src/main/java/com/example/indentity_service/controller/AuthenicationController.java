...
public class AuthenicationController {
    ...
    @PostMapping("/refresh")
    public ApiResponse<AuthenicationResponse> refresh(@RequestBody RefreshRequest request) throws ParseException, JOSEException {
       var token=  authenicationService.refreshToken(request);
        return ApiResponse.<AuthenicationResponse>builder()
                .data(token)
                .build();
    }
